...
sliderContainer.imageContainer.style.transition = '1s';
//sliderContainer.imgFrame[0].style.width = `${sliderContainer.imagesImg[0].clientWidth}px`
console.log(document.querySelectorAll('img')[0].clientWidth)
...
sliderContainer.rightArrow.addEventListener('click',(event)=>{
    const widthToPhotoOne = [...sliderContainer.imagesImg].slice(1,sliderContainer.imagesImg.length).map(e=>e.clientWidth).reduce((a,c)=>a+c)
    console.log(widthToPhotoOne)
    console.log('index',sliderContainer.currentImgIndex)
...
sliderContainer.leftArrow.addEventListener('click',(event)=>{
    ...
    sliderContainer.translateAmount+=((sliderContainer.imagesImg[sliderContainer.currentImgIndex].clientWidth))
    //update frame container width
    sliderContainer.imgFrame.style.width=`${(sliderContainer.imagesImg[sliderContainer.currentImgIndex].clientWidth)}px`
    }
    //update active dots
    for(let j =0;j<sliderContainer.dots.length;j++){
        if(sliderContainer.dots[j].classList.contains('active-dot')){
